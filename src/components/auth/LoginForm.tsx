  ...
  async function handleEmailAuth(e: React.FormEvent) {
    ...
    if (mode === 'sign_in') {
      ...
      const { error } = await supabase.auth.signUp({
        email,
        password,
        options: { emailRedirectTo: `${origin}/auth/callback?next=${redirectTo}` },
      })
      if (error) {
        toast.error(error.message)
      } else {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 border border-gray-300 rounded-lg px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition disabled:opacity-50"
      >
